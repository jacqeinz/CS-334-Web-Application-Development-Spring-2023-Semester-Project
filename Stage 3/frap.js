...
function setupTypes(db) {
  db.transaction(["frappeType"]).objectStore("frappeType").getAll().onsuccess =
    (event) => {
...
function goToFlavors(type, price) {
  ...
  window.location.href = "flavors.html?type=frap"+"&price="+ price +"+&name=" + type+ "&flavors= ";
}

function addToCart(type, price) {
  console.log(type, price);
  window.location.href="shoppingcart.html?type=frap"+"&price="+ price +"+&name=" + type+ "&flavors= ";
}
